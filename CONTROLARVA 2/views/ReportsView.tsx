...
const ReportsView: React.FC<ReportsViewProps> = ({ sales, customers }) => {
  ...
  const filteredSales = useMemo(() => {
    ...
    const start = getStartOfDay(parseDateLocal(startDate));
    const end = getEndOfDay(parseDateLocal(endDate));
    return sales.filter(sale => {
      const saleDate = parseDateLocal(sale.date);
  ...
  const chartData = useMemo(() => {
    ...
    return periodMonths.map(month => {
      const monthSales = filteredSales.filter(s => isSameMonth(parseDateLocal(s.date), month));
      return {
        name: month.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' }),
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(11);
      doc.text(`Período: ${format(parseDateLocal(startDate), 'dd/MM/yyyy')} até ${format(parseDateLocal(endDate), 'dd/MM/yyyy')}`, pageWidth - 15, 25, { align: 'right' });
      ...
      const transactions = filteredSales.map(s => [
        format(parseDateLocal(s.date), 'dd/MM/yy'),
        s.customerName,
