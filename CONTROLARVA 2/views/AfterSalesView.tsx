...
const AfterSalesView: React.FC<AfterSalesViewProps> = ({ sales, setSales, customers, interval, onNewSaleClick }) => {
  ...
  const processedItems = useMemo(() => {
    ...
    return sales.filter(sale => !sale.dismissedFromAfterSales).map(sale => {
      ...
      const postponedUntil = sale.postponedUntil ? parseDateLocal(sale.postponedUntil) : null;
              ...
              <div className="space-y-4 mb-8">
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-700 dark:text-slate-200">{format(parseDateLocal(item.date), 'dd/MM/yyyy')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-sm">
